...
function Summaries() {
    ...
    const handleAddToCart = async (summary) => {
        ...
        try {
            ...
            Swal.fire({
                ...
                html: `
          <img src="http://localhost:4000/images/${summary.summary_image}" alt="Summary Image" class="popup-image mb-5" width="265px">
          <p class="popup-title mt-3">عنوان الملخص: ${summary.summary_title}</p>
    ...
    const pageNumbers = [];
    for (let r = 1; r <= Math.ceil(summaries.length / summariesPerPage); r++) {
        pageNumbers.push(r);
    }
    ...
    useEffect(() => {
        const fetchSummariesFiltered = async () => {
            try {
                const response = await axios.get("http://localhost:4000/filtered-summaries", {
                    params: {
                        searchTerm,
                        universityFilter,
                        categoryFilter,
                ...
                console.error("There is a problem while trying to get the filtered data", error);
            }
        };
                            ...
                            <div className="col-lg-4 col-md-6 col-sm-12 mt-3" key={summary.summary_id}>
                                <div className="cardSummaries">
                                    ...
                                    <div className="card-footer mt-3">
                                        <Link className="btn btn-primary ms-3"
                                              to={`/SummaryDetails/${summary.summary_id}`}>
                                            عرض التفاصيل
                                        </Link>
